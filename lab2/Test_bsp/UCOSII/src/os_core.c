...
struct Output_buffer {
    INT32U time;
    char event;     // flag
    INT8U task1;    // preempted task
    INT8U task2;    // the chosen one
};

struct Output_buffer output_buffer[10] = {0};
int output_tail = 0;

/* lab2 */
INT32U deadline_list[65] = {0};
        ...
        if (OSIntNesting == 0) {                           /* Reschedule only if all ISRs complete ... */
            if (OSLockNesting == 0) {                      /* ... and not locked.                      */
                ...
                if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy */
                    ...
                    output_buffer[output_tail].time = OSTime;        /* lab1 */
                    output_buffer[output_tail].event = 'p';          /* lab1 */
                    output_buffer[output_tail].task1 = OSPrioCur;    /* lab1 */
                    output_buffer[output_tail].task2 = OSPrioHighRdy;/* lab1 */
                    output_tail++;                                   /* lab1 */
    ...
    if (OSIntNesting == 0) {                           /* Schedule only if all ISRs done and ...       */
        if (OSLockNesting == 0) {                      /* ... scheduler is not locked                  */
            ...
            if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy     */
                ...
                output_buffer[output_tail].time = OSTime;        /* lab1 */
                output_buffer[output_tail].event = 'c';          /* lab1 */
                output_buffer[output_tail].task1 = OSPrioCur;    /* lab1 */
                output_buffer[output_tail].task2 = OSPrioHighRdy;/* lab1 */
                output_tail++;                                   /* lab1 */
...
#endif
    /* lab2 */
    /* using EDF to replace RM */
    /* initialize */
    INT32U min = 0xffffffff;
    INT8U idx = 255;
    
    /* traverse through ready table to find the task with minimum deadline */
    int i;
    for (i = 0; i < 3; ++i) {
        unsigned j = 0;
        for (j = 0; j < 8; ++j) {
            if (OSRdyTbl[i] & (0x1<<j) && deadline_list[i*8+j] < min) {
                min = deadline_list[i*8+j];
                idx = i*8+j;
            } /* don't preempt if a task has the same deadline with current task */
            else if (OSRdyTbl[i] & (0x1<<j) && deadline_list[i*8+j] == min && idx != OSPrioCur) {
                min = deadline_list[i*8+j];
                idx = i*8+j;
            }
        }
